...
function updateSubTree(element: HTMLDetailsElement, nextState: SubTreeState): Effect {
    ...
    switch (nextState) {
        ...
        case SubTreeState.PINNED_OPEN:
            element.open = true;
            break;
    }
}

function handleSummaryClick(event: Event): Effect {
    ...
    const action = details.open ? SubTreeAction.CLOSE_LOCAL : SubTreeAction.OPEN_LOCAL;
...
function createTocContents(nextState: State): HTMLOListElement {
    ...
    if (tocType == TocType.TREE) {
        tocList = makeNestedLists(
            <GroupedHeadings>groupBy(getHeaders(), eqTagName),
            nextState
        );
    } else {
        tocList = makeFlatOrderedList(getHeaders());
    }
    return tocList;
}
...
function receiveMessage(message: { message: string }, _: Runtime.MessageSender): void {
